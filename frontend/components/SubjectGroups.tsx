...
function ProjectModal({
  ...
  // item이 undefined일 수 있으니 항상 safeItem 사용
  const safeItem: Plan = item ?? {
    plan_id: "",
    title: "",
    description: "",
    totalTime: 0,
  ...
  const handleFileUpload = useFileUpload(updateTodoFiles, safeItem.plan_id);

  const getFilteredTodos = (daily: DailyPlan) => {
    const todos = daily.todos || [];   // 안전하게 보정
    if (expandedDailyInModal === daily.daily_id) return todos;
    return [];
  };
        ...
        >
          ...
          <View style={modalStyles.header}>
            <View style={modalStyles.headerContent}>
              ...
              <Text style={modalStyles.headerTitle}>{safeItem.title}</Text>
            </View>
            <TouchableOpacity
              onPress={handleClose}
          ...
          <View style={modalStyles.timerContainerClean}>
            ...
            <Text style={modalStyles.timerBigTextClean}>
              {formatTime(safeItem.totalTime || 0)}
            </Text>
          </View>
          ...
          >
            ...
            {safeItem.dailyPlans && safeItem.dailyPlans.length > 0 ? (
              safeItem.dailyPlans.map((daily) => (
                <View
                  key={daily.daily_id}
